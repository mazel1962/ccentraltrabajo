  ...
  <div class="grid grid-rows-2 grid-cols-12 gap-0">  
    ...
    <div class="bg-yellow-400 row-start-1 row-end-2 h-16 col-start-3 col-end-11">
      <div v-for="i in 2" :key="i" >
          <div class="flex flex-row top-0 h-16 w-24">
            <div>{{userStore.empresacliente.codigoempresacliente[i-1]}}</div>
            <div>{{userStore.empresacliente.nombreempresacliente[i-1]}}</div>
            <div>{{i-1}}</div>
          </div>  
      </div>
    </div>
...
<script setup>
   ...
   import EPropietaria from './components/EPropietaria.vue';
